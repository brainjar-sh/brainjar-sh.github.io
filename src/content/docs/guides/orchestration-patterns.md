...
This page covers patterns for structuring multi-agent workflows. All patterns use `compose` as the building block — the MCP tool (and [CLI command](/reference/cli/#compose)) that assembles a full prompt from soul, persona, procedure, rules, and task, plus resolves available skills. The assembled prompt is passed to Claude Code's Agent tool to spawn the subagent. See [Subagent Orchestration](/guides/subagents/) for compose basics.
